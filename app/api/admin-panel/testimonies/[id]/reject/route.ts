...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    console.log(`[${requestId}] 🚀 Starting testimony rejection request for ID: ${testimonyId}`);
    ...
    try {
        console.log(`[${requestId}] 📥 Parsing request body as JSON...`);
        const body = await request.json();
        const { rejection_reason } = body;
        console.log(`[${requestId}] 📋 Received rejection reason:`, rejection_reason);

        // Prepare request body for external API
        const requestBody = rejection_reason ? { rejection_reason } : {};

        console.log(`[${requestId}] ✅ Validation passed. Rejecting testimony ${testimonyId}`);

        // Forward the request to the external API admin-panel endpoint
        const externalApiUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}/admin-panel/testimonies/${testimonyId}/reject/`;
        ...
        if (response.status === 200) {
            console.log(`[${requestId}] ✅ Testimony rejection completed successfully`);
        ...
        const totalDuration = Date.now() - startTime;
        console.log(`[${requestId}] ✅ Testimony rejection completed with error in ${totalDuration}ms`);
        console.log(`[${requestId}] 🎯 Returning error response with status: ${response.status}`);

        return NextResponse.json(data, { status: response.status });
    } catch (error) {
        const totalDuration = Date.now() - startTime;
        console.error(`[${requestId}] 💥 Testimony rejection error after ${totalDuration}ms:`, error);
