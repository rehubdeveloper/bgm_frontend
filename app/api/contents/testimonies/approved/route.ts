...
export async function GET(request: NextRequest) {
    ...
    console.log(`[${requestId}] 🚀 Starting APPROVED testimonies fetch request`);
    ...
    try {
        // Forward the request to the external API approved testimonies endpoint
        const externalApiUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}/contents/testimonies/approved/`;
        ...
        console.log(`[${requestId}] ✅ APPROVED testimonies fetch completed successfully in ${totalDuration}ms`);
        console.log(`[${requestId}] 🎯 Returning response with status: ${response.status}`);

        return NextResponse.json(data, { status: response.status });
    } catch (error) {
        const totalDuration = Date.now() - startTime;
        console.error(`[${requestId}] 💥 APPROVED testimonies fetch error after ${totalDuration}ms:`, error);
